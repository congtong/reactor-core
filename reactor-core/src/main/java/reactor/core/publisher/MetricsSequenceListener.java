...
/**
 * A {@link SequenceListener} that activates metrics gathering,
 * assuming Micrometer 1.x is on the classpath.
 *
 ...
 * @author Simon Baslé
 */
final class MetricsSequenceListener<T> implements SequenceListener<T> {

	final static class MetricsCompanion {

		static MetricsCompanion fromFlux(Flux<?> source) {
			Clock clock = Clock.SYSTEM;
			Tags defaultTags = DEFAULT_TAGS_FLUX;
			final String name = resolveName(source);
			final Tags tags = resolveTags(source, defaultTags);

			return new MetricsCompanion(name, tags, MicrometerConfiguration.getRegistry(), clock, false);
		}

		static MetricsCompanion fromMono(Mono<?> source) {
			Clock clock = Clock.SYSTEM;
			Tags defaultTags = DEFAULT_TAGS_MONO;
			final String name = resolveName(source);
			final Tags tags = resolveTags(source, defaultTags);

			return new MetricsCompanion(name, tags, MicrometerConfiguration.getRegistry(), clock, true);
		}
		...
		MetricsCompanion(String sequenceName, Tags tags, MeterRegistry registryCandidate, Clock clock, boolean isMono) {
			this.clock = clock;
	...
	final MetricsCompanion    companion;
	@Nullable
	...
	MetricsSequenceListener(MetricsCompanion companion) {
		...
		if (companion.isMono) {
			...
			this.onNextIntervalTimer = Timer.builder(companion.sequenceName + METER_ON_NEXT_DELAY)
				.tags(companion.commonTags)
				.description(
					"Measures delays between onNext signals (or between onSubscribe and first onNext)")
				.register(companion.registry);

			if (!REACTOR_DEFAULT_NAME.equals(companion.sequenceName)) {
				this.requestedCounter = DistributionSummary.builder(companion.sequenceName + METER_REQUESTED)
					.tags(companion.commonTags)
					.description(
	...
	@Override
	final public void doOnCancel() {
		//we don't record the time between last onNext and cancel,
		// because it would skew the onNext count by one
		recordCancel(companion.sequenceName, companion.commonTags, companion.registry, subscribeToTerminateSample);
	}

	@Override
	final public void doOnComplete() {
		...
		if (!valued) {
			recordOnCompleteEmpty(companion.sequenceName, companion.commonTags, companion.registry, subscribeToTerminateSample);
		} else if (!companion.isMono) {
			//recordOnComplete is done directly in onNext for the Mono(valued) case
			recordOnComplete(companion.sequenceName, companion.commonTags, companion.registry, subscribeToTerminateSample);
		}
	}

	@Override
	public void doOnMalformedOnComplete() {
		recordMalformed(companion.sequenceName, companion.commonTags, companion.registry);
	}

	@Override
	final public void doOnError(Throwable e) {
		//we don't record the time between last onNext and onError,
		// because it would skew the onNext count by one
		recordOnError(companion.sequenceName, companion.commonTags, companion.registry, subscribeToTerminateSample, e);
	}
	...
	public void doOnNext(T t) {
		valued = true;
		if (companion.isMono || onNextIntervalTimer == null) {
		...
		this.lastNextEventNanos = companion.clock.monotonicTime();
	...
	public void doOnSubscription() {
		recordOnSubscribe(companion.sequenceName, companion.commonTags, companion.registry);
		this.subscribeToTerminateSample = Timer.start(companion.clock);
		this.lastNextEventNanos = companion.clock.monotonicTime();
	}
	...
	static final Logger log = Loggers.getLogger(MetricsSequenceListener.class);
